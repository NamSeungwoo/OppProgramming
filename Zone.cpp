...
void Zone::DeleteAll()
{
	...
	DeleteMember dm;
	// collection 개체의 ListView에서 dm 함수 개체를 통해 순차적으로 모든 회원을 소멸 시킴
...
void Zone::AddMember()
{
	...
	int num = MyGlobal::GetNum(); // cin 포함
...
void Zone::DelMember()
{
	...
	//collection의 GetMember에서 함수 개체를 통해 원하는 회원을 가져옴
...
void Zone::SortByNumber()
{
	...
	//collection의 Sort 메서드에서는 전달받은 함수 개체를 이용하여 정렬
	collection->Sort(cn);
	ListAll();
}
